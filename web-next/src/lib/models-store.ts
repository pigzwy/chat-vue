...
function humanizeGatewayMessage(message: string) {
  ...
  if (/network fingerprint changed/i.test(message)) {
    return '登录环境与 Token 不匹配,该会话已被网关注销。请到 sub2.pigcode.ai 重新登录,并在同一浏览器里粘贴新 Token'
  }
  return message
}
...
/** 校验 sub2.pigcode.ai 会话 JWT：能拉到分组即视为有效 */
...
export async function login(rawToken: string) {
  ...
  if (!token) throw new Error('请输入 sub2.pigcode.ai 的登录 Token')
